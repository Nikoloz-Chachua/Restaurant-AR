-- 2026-08-01_segment_template_menus_reset.sql
-- Demo menu content for the four segment template tenants.
-- ROLLBACK by default; run with --commit.
--
-- DESTRUCTIVE. Deletes every menu item and category belonging to the four
-- template tenants, then re-imports. This exists to fix the first import s
-- category grouping, and is safe ONLY because these four tenants were
-- created minutes earlier by the accompanying migration and nobody has
-- edited them yet.
--
-- DO NOT run this against a tenant anyone has touched in the admin panel:
-- re-running an import over a live tenant is how Burger Bar s 3D models got
-- reset to is_3d = false once already. The WHERE clauses below are pinned to
-- these four slugs precisely so it cannot reach anything else.
BEGIN;
...
DELETE FROM menu_items WHERE restaurant_id IN
  (SELECT id FROM restaurants WHERE slug IN ('luxury', 'cafe', 'fast-casual', 'social-dining'));
