...
// demo hostnames; v157 restored the FOOD | DRINKS split; v158 named the cafe
// demo. (v155 was this branch's number before merging v154; skipped so the
// published cache only ever moves forward.)
test('service worker cache advances and precaches MINGLEYARD editorial CSS', () => {
  assert.match(sw, /const CACHE_NAME = 'bl-v158';/);
  assert.equal((sw.match(/bl-v158/g) || []).length, 1);
